...
harness/flex-update.sh                   # `update` AVEC install sur un projet Flex (tranches B et C), 15 cas : natif quand aucun paquet enregistré n'a de recette ni de classe de bundle — la classe lue dans le dist pour un paquet que l'install pose (`dist-bundle`, `dist-no-bundle`), dans vendor/ sinon ; le rappel `symfony/thanks` sur une opération d'update (`thanks`) ; repli pour une recette, un bundle, une suppression (`remove-package`), symfony/flex installé par le run (`flex-installed`), et les six cas `sync-*` du garde de synchronisation (importmap.php ou package.json sans paquet `symfony-ux` : natif ; paquet `symfony-ux`, assets/controllers.json, lien `file:` mort, fichier non normalisé : repli). ATTENTION : toute retouche de l'émulation de Flex doit aussi faire tourner transitions.sh et vendor-dir.sh, qui affirment ses motifs de repli
