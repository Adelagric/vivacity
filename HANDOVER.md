...
harness/flex-update.sh                   # `update` AVEC install sur un projet Flex (tranches B et C) : natif quand rien de neuf n'est posé et qu'aucune recette ne s'applique, repli (rien d'écrit) pour une recette de l'index, une classe de bundle, ou un paquet à poser ; et les six cas `sync-*` du garde de synchronisation (importmap.php ou package.json sans paquet `symfony-ux` : natif ; paquet `symfony-ux`, assets/controllers.json, lien `file:` mort, fichier non normalisé : repli)
